...
const { sanitizeRichText, isRichTextEmpty } = require('../utils/sanitize');
...
async function createArticle(req, res) {
  try {
    ...
    const cleanSummary = summary ? sanitizeRichText(summary, {
      allowedTags: [],
      allowedAttributes: {}
    }) : '';
    ...
    const [result] = await pool.query(
      `INSERT INTO articles (title, content, summary, cover_image, author_id, status) 
       VALUES (?, ?, ?, ?, ?, ?)`,
      [title, cleanContent, cleanSummary, cover_image || '', authorId, status]
    );
