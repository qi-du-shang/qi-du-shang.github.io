...
async function getUserInfo(req, res) {
  try {
    ...
    const [users] = await pool.query(
      'SELECT id, username, nickname, avatar, bio, role, created_at, last_login_at, follower_count, following_count FROM users WHERE id = ?',
      [userId]
    );
    ...
    const [totalViews] = await pool.query(
      'SELECT SUM(view_count) as total FROM articles WHERE author_id = ? AND status = "published"',
      [userId]
    );

    // 获取用户阅读过的文章数量
    const [readCountResult] = await pool.query(
      'SELECT COUNT(*) as count FROM article_reads WHERE user_id = ?',
      [userId]
    );
...
async function searchUsers(req, res) {
  try {
    ...
    const [users] = await pool.query(
      `SELECT id, username, nickname, avatar, bio, follower_count, following_count
       FROM users 
...
async function getUserList(req, res) {
  try {
    ...
    const [users] = await pool.query(
      `SELECT id, username, nickname, avatar, role, status, created_at, last_login_at, follower_count, following_count
...
async function toggleUserStatus(req, res) {
  try {
    ...
    // 不允许禁用其他管理员
    if (users[0].role === 'admin' && status === 1) {
      return res.status(400).json({
        code: 400,
        message: '不能禁用其他管理员账号'
      });
