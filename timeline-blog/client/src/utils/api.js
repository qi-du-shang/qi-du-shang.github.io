...
export function getApiUsageOverview() {
  return request.get('/admin/api-usage/overview')
}
export function getApiUsageByApi(params) {
  return request.get('/admin/api-usage/by-api', { params })
}
export function getApiUsageTrend(days = 30) {
  return request.get('/admin/api-usage/trend', { params: { days } })
}
export function getApiUsageIpDistribution(params) {
  return request.get('/admin/api-usage/ip-distribution', { params })
}
export function getApiUsageAlerts() {
  return request.get('/admin/api-usage/alerts')
}
export function getApiUsageLogs(params) {
  return request.get('/admin/api-usage/logs', { params })
}
...
export function resetUserPassword(userId, newPassword) {
  return request.put(`/users/${userId}/reset-password`, { newPassword })
}

// 管理员：启用/禁用某用户（status: 0正常 / 1禁用）
export function updateUserStatus(userId, status) {
  return request.put(`/users/${userId}/status`, { status })
}

// 前端上报访问记录
export function reportVisit(path, referer) {
  return request.post('/visitor-logs/report', { path, referer })
}

// 管理员：访客记录列表
export function getVisitorLogs(params) {
  return request.get('/visitor-logs/logs', { params })
}

// 管理员：访客统计概览
export function getVisitorStats() {
  return request.get('/visitor-logs/stats')
}
