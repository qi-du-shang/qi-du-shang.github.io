        ...
        <div class="security-section card">
          ...
          <div class="security-content">
            <div class="form-group">
              <label>旧密码</label>
              <input type="password" v-model="pwdForm.oldPassword" class="input" placeholder="请输入当前密码" autocomplete="current-password" />
            </div>
            <div class="form-group">
              <label>新密码</label>
              <input type="password" v-model="pwdForm.newPassword" class="input" placeholder="6-32位，建议混合字母与数字" autocomplete="new-password" />
            </div>
            <div class="form-group">
              <label>确认新密码</label>
              <input type="password" v-model="pwdForm.confirmPassword" class="input" placeholder="再次输入新密码" autocomplete="new-password" />
            </div>
            <div class="security-actions">
              <button class="btn btn-primary" @click="handleChangePassword" :disabled="pwdSaving">
                {{ pwdSaving ? '提交中...' : '修改密码' }}
              </button>
            </div>
          </div>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <RichTextEditor
              ...
              mode="full"
              :height="400"
              placeholder="在这里写下你的文章，支持富文本格式、图片、代码块、表格等..."
            />
          </div>
        </div>
        <div class="modal-footer">
...
async function saveArticle(status) {
  ...
  // 富文本内容：去除 HTML 标签后判断是否为空
  const plainText = articleForm.value.content.replace(/<[^>]*>/g, '').trim()
  if (!plainText) {
