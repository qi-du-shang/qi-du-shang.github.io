        ...
        <aside class="sidebar-left">
          <div class="user-card card">
            ...
            <div class="user-stats">
              ...
              <div v-if="userStore.isLoggedIn" class="stat-item" style="cursor: pointer;" @click="openFollowList">
        ...
        <section class="timeline-section">
          ...
          <div class="timeline-container" ref="timelineContainer">
            ...
            >
              ...
              <div class="timeline-card card fade-in card-hover magnetic-target" @click="goToArticle(article.id)" :style="{ animationDelay: (index % 6) * 0.08 + 's' }">
                <div class="card-date">
        ...
        <div class="modal-body">
          ...
          <div v-else class="follow-user-list">
            <div 
              ...
              @click="goToUserHomeFromModal(user.id)"
            >
...
import { getArticleList, searchArticles, searchUsers, getUserMusicSettings, getFollowing } from '../utils/api'
...
const stats = computed(() => {
  if (userStore.isLoggedIn && userStore.userInfo) {
    return {
      ...
      readCount: userStore.userInfo.readCount || 0,
      totalViews: myArticles.value.reduce((sum, item) => sum + (item.view_count || 0), 0)
    }
  }
  return {
...
// 从关注列表弹窗跳转到用户主页
function goToUserHomeFromModal(userId) {
...
async function fetchFollowingList() {
  ...
  try {
    const res = await getFollowing(userStore.userInfo.id, { page: 1, pageSize: 50 })
    if (res.code === 200) {
...
@keyframes dotPulse {
  0%, 100% { box-shadow: 0 0 8px var(--timeline-line); transform: scale(1); }
  50% { box-shadow: 0 0 18px var(--timeline-line), 0 0 30px rgba(0, 212, 255, 0.3); transform: scale(1.15); }
}

/* 时间轴卡片入场交错 */
.timeline-item {
  animation: timelineIn 0.6s cubic-bezier(0.16, 1, 0.3, 1) backwards;
}
