...
- 📝 **文章管理** - 文章的增删改查
- 🔐 **权限管理** - 角色和权限管理
- 👥 **账号管理** - 用户列表、搜索、管理员重置密码、启用/禁用账号
- 👣 **访客记录** - 记录每个访问者的 IP、设备、浏览器、操作系统、访问路径；登录用户自动关联身份；今日/累计统计 + 近7天趋势
- 📈 **接口用量统计** - 全接口调用量监控、趋势分析、异常告警（仅管理员可见）
- 🟢 **实时在线人数** - 管理后台顶栏实时显示当前在线总人数、会员/游客分布（Socket.io 推送）

### 个人中心
- 🔒 **修改密码** - 个人主页内置"账号安全"卡片，校验旧密码后修改新密码
...
## ✨ V6 升级说明：账号管理 + 访客记录 + 在线人数

本次升级在不改变原有框架的前提下，新增以下能力：

### 新增功能

1. **修改密码**
   - 位置：个人中心（`/profile`）→ "🔒 账号安全"卡片
   - 必须输入旧密码，新密码 ≥ 6 位，两次输入一致

2. **账号管理（管理后台 → `/admin/accounts`）**
   - 用户列表：头像、用户名、昵称、角色、状态、注册时间、最后登录
   - 支持按用户名/昵称搜索、分页
   - 管理员可重置任意用户密码（不能重置自己）
   - 管理员可启用/禁用用户（不能禁用自己、不能禁用其他管理员）
   - 被禁用账号登录时会被拒绝

3. **访客记录（管理后台 → `/admin/visitors`）**
   - 前端路由切换时自动上报访问（路径、来源、IP、UA）
   - 自动解析浏览器、操作系统、设备类型（PC/Mobile/Tablet）
   - 登录用户自动关联 user_id，游客标记为"游客"
   - 统计：今日访问、今日独立 IP、今日登录访客、累计访问、累计独立 IP
   - 近 7 天访问趋势图
   - 支持按关键词、用户类型（全部/登录/游客）筛选

4. **实时在线人数**
   - 基于 Socket.io 内存维护在线连接
   - 管理后台顶栏实时显示：在线总数、会员数、游客数
   - 登录用户连接时自动上报身份（通过握手 token + identify 事件）

### 数据库迁移

```bash
mysql -u root -p < database/migration_v6.sql
```

该脚本会：
- 给 `users` 表新增 `status` 字段（0 正常 / 1 禁用）
- 创建 `visitor_logs` 访客访问记录表

### 升级步骤

1. 执行 `database/migration_v6.sql`
2. 后端：`cd server && npm install`（无新增依赖）
3. 前端：`cd client && npm install`（无新增依赖）
4. 重启前后端服务

### 新增/修改的文件

- `database/migration_v6.sql`（新增）
- `server/controllers/userController.js`（新增 changePassword / resetUserPassword / toggleUserStatus）
- `server/controllers/visitorLogController.js`（新增）
- `server/routes/userRoutes.js`（新增 3 条路由）
- `server/routes/visitorLogRoutes.js`（新增）
- `server/app.js`（注册路由 + Socket 在线用户管理）
- `client/src/utils/api.js`（新增 6 个 API）
- `client/src/utils/socket.js`（握手带 token + 在线人数事件）
- `client/src/store/user.js`（userInfo 持久化到 localStorage）
- `client/src/router/index.js`（新增 2 个管理路由 + afterEach 上报访问）
- `client/src/main.js`（启动时初始化 Socket）
- `client/src/views/Profile.vue`（新增修改密码卡片）
- `client/src/views/admin/AdminLayout.vue`（新增菜单项 + 在线人数徽章）
- `client/src/views/admin/AccountManage.vue`（新增）
- `client/src/views/admin/VisitorLog.vue`（新增）
