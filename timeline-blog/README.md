...
│   │   ├── assets/         # 静态资源
│   │   │   └── styles/     # 全局样式
│   │   ├── components/     # 公共组件（含 MagneticPointer 磁吸光标）
...
> **注意**：全新安装需依次执行 init.sql → migration_v2.sql → migration_v3.sql → migration_v4.sql。
> 若已执行过前面的脚本，只需执行 migration_v4.sql 即可启用接口用量统计功能。
