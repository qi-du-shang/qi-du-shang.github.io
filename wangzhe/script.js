...
const areaTypeSelect = $("#area-type");
...
const areaTypeNames = { 1: "省榜", 2: "市榜", 3: "区榜" };
...
function renderRanking(result) {
  ...
  const levelName = areaTypeNames[Number(result?.level?.level)] || areaTypeNames[Number(areaTypeSelect.value)] || "榜单";

  $("#result-hero").textContent = heroName;
  $("#result-level").textContent = levelName;
  $("#table-level").textContent = levelName;
  ...
  $("#result-count").textContent = `${list.length} 条排名`;
...
async function search() {
  ...
  try {
    ...
    const selectedResult = {
      ...
      2: cityResult,
      3: districtResult
    }[Number(areaTypeSelect.value)] || provinceResult;
    renderRanking(selectedResult);
...
areaTypeSelect.addEventListener("change", () => {
  if (heroInput.value.trim()) search();
});
